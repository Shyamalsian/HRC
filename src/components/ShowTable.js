...
const ShowTable = ({ row }) => {
  ...
  const [editSelectedRow,setEditSelectedRow] = useState({order_currency:'',company_code:'',distribution_channel:''});
  ...
  const columns = [
    ...
    { field: 'amountUSD', headerName: 'AMOUNT USD', width: 180 },
  ];
  ...
  const handleEditModal = () => {
    const updatedSelectedRowData = {
      ...selectedRowData,
      orderCurrency: editSelectedRow.order_currency,
      companycode: editSelectedRow.company_code,
      distribution: editSelectedRow.distribution_channel
    };
      ...
      <ThemeProvider theme={theme}>
        ...
        <Dialog open={editModalOpen} onClose={handleEditModalClose}>
              ...
              <div style={{ width: "50%" }}>
                <Stack sx={{ flexDirection: "row", width: "300px", justifyContent: "space-between", paddingTop: "8px" }}>
                  ...
                  <TextField
                    ...
                    maxRows={1}
                    sx={{ paddingRight: "5px" }}
                    value={editSelectedRow.order_currency}
                    onChange={e=> setEditSelectedRow({...editSelectedRow,order_currency:e.target.value})}
                  />
                  <TextField
                    id="outlined-multiline-flexible"
                    label="Company Code"
                    multiline
                    maxRows={1}
                    value={editSelectedRow.company_code}
                    onChange={e=> setEditSelectedRow({...editSelectedRow,company_code:e.target.value})}
                  />
                ...
                <Stack sx={{ width: "300px", paddingTop: "10px" }}>
                  <TextField
                    ...
                    multiline
                    maxRows={1}
                    value={editSelectedRow.distribution_channel}
                    onChange={e=> setEditSelectedRow({...editSelectedRow,distribution_channel:e.target.value})}
                  />
                </Stack>
          ...
          <DialogActions>
            ...
            <Button onClick={handleEditModal} variant="outlined" sx={{ width: "150px", marginRight: "15px", color: 'black', border: '0.5px solid black' }}>
              Save Changes
            </Button>
          </DialogActions>
        </Dialog>
